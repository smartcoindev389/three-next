...
import ReferralGroup from "@/assets/icons/inline-svg/referral.svg";
import ReferralPopup from "@/shared/components/(user)/Referral/ReferralPopup";
...
const ReferralLinks: FC = () => {
  const { t } = useTranslation();
  const [copySuccess, setCopySuccess] = useState("");
  const inputRef = useRef(null);
  const [isOpen, setIsOpen] = useState(false);
  const { data, refetch, loading } = useQuery(REFERRAL_FACTORY_CONFIG);
  const referralLink =
    siteUrl + "refer/" + data?.referralFactoryCustomerConfig.code;
  ...
  return (
    <Card className="xl:flex h-full">
      <ReferralGroup className="shrink-0 mx-auto" />
      <div className="flex flex-col items-center w-full">
        <p className="text-[#495057] text-[45px] md:text-[48px] font-din-condensed font-bold">
          <Trans t={t}>Share & Earn</Trans>
        </p>
        <p className="mt-0 text-base md:text-lg text-[#495057] text-[18px] font-semibold">
          <Trans t={t}>Unique Referral Link</Trans>
        </p>
        <section className="bg-white mt-3 w-full">
          <div className="mx-auto w-full max-w-[470px]">
            {loading ? (
              ...
              <div className="relative">
                <input
                  ref={inputRef}
                  className="h-12 w-full md:text-lg rounded-lg border border-stroke bg-gray-1 py-3 pl-5 pr-28 text-[#808080] outline-none duration-200 focus:border-primary"
                  value={referralLink}
                  disabled={!data.is_active}
                  type="text"
                />
                <button
                  className="text-[16px] absolute right-1 top-1/2 inline-flex h-10 -translate-y-1/2 items-center justify-center gap-1 rounded-md bg-[#00C0F3] px-4 py-1.5 text-sm font-[500] text-white duration-200 hover:bg-dark/90"
                  disabled={!data?.referralFactoryCustomerConfig.is_active}
                  onClick={copyToClipboard}
                >
                  {copySuccess ? t("Copied") : t("Copy Link")}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="text-center py-4">
          <button
            className="font-normal md:text-lg text-sky-400 underline  hover:text-sky-500 transition-colors duration-200"
            type="button"
            onClick={() => setIsOpen(true)}
          >
            <Trans t={t}>Customize my Unique Referral Link</Trans>
          </button>
          {isOpen && (
            <ReferralPopup
        ...
        <div className="flex gap-2 md:gap-4 items-center mt-1">
          ...
          <FacebookMessengerShareButton appId="FUR4pets" url={referralLink}>
            <svg
              fill="none"
              height="38"
              viewBox="0 0 38 38"
            ...
            >
              ...
              />
              <path
                d="M18.9899 6.54837C12.1186 6.54837 6.54785 11.7194 6.54785 18.0992C6.54785 21.7342 8.35665 24.9764 11.1839 27.0941V31.4996L15.4204 29.1681C16.5511 29.4813 17.7486 29.6515 18.9899 29.6515C25.861 29.6515 31.4317 24.4804 31.4317 18.1007C31.4317 11.7195 25.8609 6.54837 18.9899 6.54837Z"
                fill="white"
              />
              <path
                d="M20.2269 22.1037L17.0588 18.715L10.877 22.1037L17.6763 14.8648L20.922 18.2536L27.0279 14.8648L20.2269 22.1037Z"
                fill="#0097FF"
              />
            </svg>
          </FacebookMessengerShareButton>
